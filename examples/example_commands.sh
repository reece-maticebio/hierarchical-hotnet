#!/usr/bin/env bash
#
# Run the Hierarchical HotNet pipeline on the example data.
#
# This script assumes the package has been installed (e.g. `pip install .`
# from the repository root), which exposes the CLI commands used below.
# Hierarchical HotNet is parallelizable; see example_commands_parallel.sh for
# a parallelized variant.
