#!/usr/bin/env bash
#
# Run the Hierarchical HotNet pipeline on the example data in parallel.
#
# This script assumes the package has been installed (e.g. `pip install .`
# from the repository root) and that GNU parallel is on PATH.
# See https://www.gnu.org/software/parallel/ to install GNU parallel.
# Adjust num_cores below to match your system.
...
mkdir -p "$data" "$intermediate" "$results"
